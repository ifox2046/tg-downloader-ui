...
- 进行中的任务记录暂停请求，并进入 `paused` 状态。
- `tdl` PID 保持不变，Linux 进程状态为 stopped。
- 已有临时文件不会被重新创建或截断，暂停期间文件大小保持不变。
...
- 继续操作向同一个 PID 发送 `SIGCONT`，不会启动新的 `tdl` 命令，也不会增加尝试次数。
- 恢复时保留已有进度和已下载大小字段，临时文件从暂停时的字节位置继续增长。

应用或容器重启后的恢复是另一种语义：失败/取消后仍保留
`tdl download --continue -f <export.json>`，用于跳过多项目任务中已完成的项目；
但 `tdl 0.20.3` 不支持单个未完成文件的字节级续传，不能把容器/应用重启描述为
单文件断点续传。
