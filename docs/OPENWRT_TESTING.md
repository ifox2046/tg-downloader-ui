...
When testing the generic `tg-downloader-ui_0.1.4_all.ipk` package, install
...
scp .\dist\openwrt\tg-downloader-ui-full_0.1.4_x86_64.ipk "${target}:/tmp/"
ssh $target "opkg install /tmp/tg-downloader-ui-full_0.1.4_x86_64.ipk"
```
...
# artifact: dist/openwrt/tg-downloader-ui-full_0.1.4_aarch64_generic.ipk
# Architecture: aarch64_generic ; tdl asset: tdl_Linux_arm64.tar.gz
```

```powershell
$target = "$env:OPENWRT_TEST_USER@$env:OPENWRT_TEST_HOST"
scp .\dist\openwrt\tg-downloader-ui-full_0.1.4_aarch64_generic.ipk "${target}:/tmp/"
ssh $target "opkg install /tmp/tg-downloader-ui-full_0.1.4_aarch64_generic.ipk"
```
...
```powershell
$target = "$env:OPENWRT_TEST_USER@$env:OPENWRT_TEST_HOST"
scp .\dist\openwrt\tg-downloader-ui_0.1.4_all.ipk "${target}:/tmp/"
ssh $target "opkg install /tmp/tg-downloader-ui_0.1.4_all.ipk"
```
