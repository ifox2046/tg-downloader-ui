...
Web UI 支持**中文**与**英文**。页面顶部（登录页、首次初始化页同样提供）可用
**中文 | EN** 切换。偏好保存在浏览器 `localStorage`（键名 `tgdl_lang`）。
未保存偏好时，按浏览器 `navigator.languages` 匹配 `zh*` / `en*`，否则默认中文。
服务端 API 错误信息保持英文机器文案，客户端原样展示。转发频道摘要中的
`File` / `Size` / `Message ID` 固定为英文，不受 UI 语言切换影响。
...
| 控制 Bot（私聊入队） | Web UI 管理员 + `tdl` 登录 + BotFather token（「控制 Bot」页） |
...
- `tg-downloader-ui_0.1.4_all.ipk`：与架构无关的应用包，需要另行安装与设备架构匹配的上游 `tdl`。
- `tg-downloader-ui-full_0.1.4_x86_64.ipk`：x86_64 完整包，包含应用和未经修改的上游 `tdl 0.20.3` 二进制文件（`tdl_Linux_64bit.tar.gz`）。
- `app-meta-tg-downloader-ui_0.1.4-r1_all.ipk`：iStore 已安装应用元数据包。
...
使用 `--full-arch all` 时还会生成 `tg-downloader-ui-full_0.1.4_aarch64_generic.ipk`。不同 CPU 架构的 full 包是独立 IPK 文件，包名同为 `tg-downloader-ui-full`，并 Conflicts/Provides `tg-downloader-ui`。
...
本项目自身代码采用 MIT 许可证。多架构 Docker 镜像（`linux/amd64` 与 `linux/arm64`）以及 full OpenWrt IPK（`tg-downloader-ui-full_0.1.4_x86_64.ipk` 与 `tg-downloader-ui-full_0.1.4_aarch64_generic.ipk`）内置的 `tdl` 是未经修改的上游 `tdl 0.20.3` 二进制文件，采用 AGPL-3.0 许可证。每个 full IPK 会在 `/usr/share/licenses/tg-downloader-ui-full` 中安装上游许可证及源码/版本声明。详情参阅 [THIRD_PARTY.md](THIRD_PARTY.md)。
